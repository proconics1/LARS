...
/**
 * This class contains the program logic. The user interface communicates with this class to load and save data.
 * In addition, the Backend class is the topmost node of the tree, which is explained in {@link Layer}.
 * 
 *
 */
public class Backend extends Observable implements MutableTreeNode, ScoreListener {
	...
	private static final boolean CHECKDB = true; //Run through the tree and check for consistency
	...
	private Backend() throws InvalidDatabaseStructureException {
		...
		//Check that only categories are questions.
		for(Layer l : leafs) {
			if(!(l instanceof Category)) {
				throw new InvalidDatabaseStructureException(l + " is not a category.");
			}
		}
	}
	...
	/**
	 *Gets the kids at a certain point
	 * @param path,Path where children are to be retrieved
	 * @return Returns the children of the element path
	...
	public Layer[] getElements(Layer path) throws InvalidLayerStackException {
		if(path == null) {
			...
			throw new IllegalArgumentException("Unknown Layer Class " + path.getClass());
		}
	}
	...
	public Layer[] getCurrentElements() throws BackendInconsistencyException {
		try {
			...
			throw new BackendInconsistencyException("Invalid layer combination");
		}
	}
...
//			int ix = currentLayer.getParent().getIndex(currentLayer);
	...
	public Comment getComment(Layer layer) throws InvalidDatabaseStructureException, NoCaseSelectedException, NoUserSelectedException {
		...
		if(layer instanceof Category) {
			...
			throw new IllegalArgumentException(layer.getClass() + " may have no answer.");
		}
	}
	...
	public String getStatusDefaultComment(String status) {
		...
		throw new IllegalAccessError("Unsupported Status.");
	}

	public String getUserName(int user_id) throws InvalidDatabaseStructureException {
		User user = data.getUser(user_id);
		if(user == null) {
			throw new InvalidDatabaseStructureException("No user for Id " + user_id);
		}else{
